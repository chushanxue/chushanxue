...
> 拓展：为什么父组件更新子组件也会更新？在 React 中，函数组件本质是一次函数调用，父组件重新执行时，子组件也会重新执行，所以默认都会触发 render。
...
> 拓展：注意useRef也可以保存函数引用（函数必须永远不变的场景）。比如当你把一个函数传给第三方库、原生事件监听器、非 React 感知的系统时，如果函数引用频繁变化，这可能会导致重复注册、内存泄露、性能问题。
