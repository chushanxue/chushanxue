...
通常情况下，App 内部会使用 WebView 控件作为网页引擎。这是系统自带的控件，专门用来显示网页。应用程序的界面，只要放上 WebView，就好像内嵌了**浏览器窗口**，可以显示网页。

> 它像一个没有地址栏和导航栏的简化版浏览器,通过JavaScript Bridge（JSBridge）实现与原生代码的双向通信，从而调用原生功能或获取数据

不同的 App 技术栈要显示网页，区别仅仅在于怎么处理 WebView 这个原生控件。它本质上是操作系统（Android用Chromium, iOS用WebKit）提供的浏览器引擎，集成在App的UI布局中。<mark>通过这个内核加载、解析、渲染HTML/CSS/JS</mark>

核心组成：

- 浏览器内核：负责网页渲染，解析DOM/CSSOM生成渲染树，并绘制到屏幕上。
- JSBridge：连接原生代码与JS的“桥梁”，通过注入全局对象（如window.JSBridge），允许JS调用原生API（**访问GPS、摄像头、支付**等设备功能。），原生代码也可调用JS函数。
- WebViewClient/WebChromeClient：原生回调，处理页面加载、错误、UI事件（如标题、对话框）等。
