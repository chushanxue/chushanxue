...
- 选型：主要是语音识别速度和精确识别能力、具备流式传输能力(可以边说话边识别)
- 官网下载下来的Demo需要调用很多第3方库，而且还要经过打包编译，过于复杂
- 在社区找到二次封装的解决方案，去除了很多不必要的插件和代码，可以直接在Web前端PC端、移动H5中引入使用（不用去打包编译）

### 外部代码兼容

[参考链接](https://blog.csdn.net/muguli2008/article/details/106734113)
...
export default class IatRecorder {
  ...
  connectWebSocket() {
    return this.getWebSocketUrl().then((url) => {
      ...
      iatWS.onclose = (e) => {
        console.log('讯飞ws连接关闭');
        // 在外部js/ts中使用时 pinia还没有完成初始化，所以引入后不要立马声明
        // const store = useChatStore();
        // store.$patch({ stopXunfei: true });
        this.recorderStop(e);
      };
    });
  }
...
const voice = new Voice({
  ...
  onTextChange: function (text: string) {
    //监听识别结果的变化
    //isRecord是业务变量，代表是否开启录音按钮
    if (isRecord.value) {
      content.value = text;
      emit('updateMessage', text); //父组件输入内容同步更新
    }
  },
});
