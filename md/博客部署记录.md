...
> ⚠️这点非常重要，在部署的时候，必须先在本地跑一遍`pnpm build:pre`，所以才需要使用`fast.sh`方案，因为本质上提交的是docs文件夹，今天踩到了这个坑，发现部署的时候出错，进而去维护`.github/workflows/deploy.yml`，摸索了一阵发现其实根本不需要这一步，因为build操作在本地做而不是线上，并且线上之前也没有维护过
